    ...
    <section className="panel chat-panel">
      ...
      <form className="question-form" onSubmit={submit}>
        <input
          ...
          placeholder="Ask your paper a question..."
          aria-label="Question"
        />
        <button
      ...
      {answer ? (
        <div className="answer-block">
          <div className="answer-label">
            <span className="answer-mark">✦</span> PaperReader's answer{" "}
            {documentName && (
              <span className="document-tag">
                <BookOpen size={12} /> {documentName}
              </span>
            )}
          </div>
          ...
          {sources.length > 0 && (
            <div className="sources">
              ...
              {sources.map((source, index) => (
                <div className="source" key={`${source}-${index}`}>
                  {source}
                </div>
              ))}
            </div>
        ...
        <div className="empty-chat">
          <BookOpen size={22} />
          <span>Your answer will appear here, grounded in the text.</span>
        </div>
      )}
    </section>
  );
